...
func DispatchBlob(meta *BlockData, game *GameState, blob *simplejson.Json) {

	if len(meta.blockTitle) > 0 {
		if meta.blockTitle == "Event.DeckSubmit" ||
			meta.blockTitle == "Event.GetPlayerCourse" {
			ParseEventDecksubmit(game, blob)
		}
	}
